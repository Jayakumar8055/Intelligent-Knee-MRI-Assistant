...
function AdminSaves() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(
        `http://127.0.0.1:5000/api/adminSends/metadata?email=${userEmail}`
      );
    ...
    try {
      ...
      if (answer && answer.toLowerCase() === "yes") {
        const res = await axios.delete(
          `http://127.0.0.1:5000/api/admin_sends_delete/${id}`
        );
  ...
  return (
    <div>
      <h1 className="text-[20px] text-center my-5 text-gray-700">Reports sended to the patients</h1>
      <div>
        <input
          ...
          <tbody>
            {filterMetaData.map((item, index) => (
              <tr
                ...
                // className={`$index % 2 === 0 ? 'bg-gray-50 dark:bg-gray-900' : 'bg-white dark:bg-gray-900' border-b dark:border-gray-400`}
              >
                ...
                <td
                  className="px-6 py-4 font-medium text-gray-600 whitespace-nowrap text-lg text-[25px] dark:text-black"
